...
class KjørGrunnbeløpsreguleringRiverTest {
    ...
    @Test
    fun `kjører ikke grunnbeløpsregulering med ugyldig periode`() {
        testRapid.sendTestMessage(event("kjør_grunnbeløpsregulering", 101_000.0, tom = LocalDate.parse("2023-01-01")))
        verify(exactly = 0) {
            anvendtGrunnbeløpDao.hentFeilanvendteGrunnbeløp(any(), any())
        }
    }
    ...
    @Language("JSON")
    private fun event(eventName: String, riktigGrunnbeløp: Double, tom: LocalDate? = null): String = """{
